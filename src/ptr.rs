...
fn align_offset_various_strides() {
    unsafe fn test_stride<T>(ptr: *const T, align: usize) -> bool {
        ...
        for el in 0..align {
            if (numptr + el * ::core::mem::size_of::<T>()) % align == 0 {
                expected = el;
                break;
            }
        }
...
fn ptr_metadata() {
    ...
    unsafe {
        let dst_tuple: &(bool, str) = core::mem::transmute(dst_tuple);
    ...
    unsafe {
        ...
        let address_5: *const () = core::mem::transmute(vtable_5);
...
fn dyn_metadata() {
    ...
    assert_eq!(meta.size_of(), core::mem::size_of::<Something>());
    assert_eq!(meta.align_of(), 32);
    assert_eq!(meta.align_of(), core::mem::align_of::<Something>());
